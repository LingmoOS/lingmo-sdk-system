...
typedef struct process_info
{
    ...
    char proc_command[400]; //进程启动的命令行,包含启动参数
...
char **kdk_procname_get_process_infomation(char *proc_name)
{
    ...
    while (pid[index])
    {
        ...
        proc_pid = atoi(proc);
        res[index] = malloc(sizeof(char) * PROC_COMD_NUM);
        if (!res[index])
        {
            klog_err("内存申请失败:%s\n", strerror(errno));
            goto err;
        ...
        // memset(tmp, 0, PROC_COMD_NUM * sizeof(char));
        ...
        char *proc_state = kdk_get_process_status(proc_pid);
        ...
        char *start_time = kdk_get_process_start_time(proc_pid);
        strcat(result, "start_time:");
        strcat(result, start_time);
        strcat(result, ", ");
        free(start_time);

        char *proc_time = kdk_get_process_running_time(proc_pid);
        strcat(result, "proc_time:");
        strcat(result, proc_time);
        strcat(result, ", ");
        free(proc_time);

        char *cpu_time = kdk_get_process_cpu_time(proc_pid);
        strcat(result, "utime:");
        strcat(result, cpu_time);
        strcat(result, ", ");
        free(cpu_time);

        char *proc_command = kdk_get_process_command(proc_pid);
