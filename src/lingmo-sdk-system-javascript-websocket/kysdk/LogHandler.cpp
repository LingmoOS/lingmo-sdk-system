...
LogHandlerPrivate::LogHandlerPrivate() {
    ...
    QString logfile_name = homePath + "/.log/peripheral/";//日志路径：~/.log/peripheral/
    if(!logDir.exists(logfile_name))
    {
          bool res = logDir.mkpath(logfile_name);
          qDebug()<<"创建日志 "<<res;
    }
    logDir.setPath(logfile_name); // TODO: 日志文件夹的路径，为 exe 所在目录下的 log 文件夹，可从配置文件读取
