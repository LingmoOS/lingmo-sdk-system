...
static void *printDate(DBusConnection *ptr)
{
	...
	if (g_DateChanged == 1)
	{
		...
		msg = dbus_message_new_signal("/com/lingmo/kysdk/Date",
									"com.lingmo.kysdk.DateInterface",
									"DateSignal");
		if (NULL == msg)
		{
...
const char *server_introspection_xml =
	...
	"  <interface name='com.lingmo.kysdk.DateInterface'>\n"
...
int main()
{
	...
	rv = dbus_bus_request_name(conn, "com.lingmo.kysdk.DateServer", DBUS_NAME_FLAG_REPLACE_EXISTING, &err);
	...
	if (!dbus_connection_register_object_path(conn, "/com/lingmo/kysdk/Date", &server_vtable, NULL))
	{
