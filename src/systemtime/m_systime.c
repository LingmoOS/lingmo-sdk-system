...
static void *printClock(void *ptr)
{
	...
	if (g_TimeChanged == 1)
	{
		...
		msg = dbus_message_new_signal("/com/lingmo/kysdk/Timer",
									  "com.lingmo.kysdk.TimeInterface",
									  "TimeChangeSignal");
		if (NULL == msg)
		{
...
const char *server_introspection_xml =
	...
	"  <interface name='com.lingmo.kysdk.TimeInterface'>\n"
...
int main(void)
{
	...
	rv = dbus_bus_request_name(conn, "com.lingmo.kysdk.TimeServer", DBUS_NAME_FLAG_REPLACE_EXISTING, &err);
	...
	if (!dbus_connection_register_object_path(conn, "/com/lingmo/kysdk/Timer", &server_vtable, NULL))
	{
